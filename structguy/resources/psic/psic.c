...
Usage: `psic [options] aln_file matrix_file [out_file]'
...
#define MAXSEQLEN  140960
#define MAXSEQNUM   9998192
...
static const char *optString = "lp:q:";
...
int      use_sqrt_neff = 1;
int      log_probs = 0;  /* use log(probs) as PSIC value instead of log(probs/background_freqs) */
double   lambda = 0.225;
...
int main(int argc, char* argv[])
{
  ...
  while( opt != -1 ) {
    switch ( opt ) {
      ...
      case 'p':
        ...
        if (pseudocount <= 0 || pseudocount > 100) {
          printf("Error: supplied -p value (%.0f) is out of range [1..100]\n", pseudocount);
          return 1;
        }
        break;
      case 'q':
        use_sqrt_neff = atoi(optarg);
        if (use_sqrt_neff!=0 && use_sqrt_neff!=1) {
          printf("Error: supplied -q value (%d) is out of range [0|1]\n", use_sqrt_neff);
          return 1;
        }
        break;
      default:
        printf("Usage: %s [-l] [-p X] [-q 1|0] aln_file matrix_file [out_file]\n", argv[0]);
        return 1;
    }
    opt = getopt( argc, argv, optString );
  }

  int arrgc = argc - optind;
	...
	if (read_matr_file(argv[optind+1])!=AL_SIZE*AL_SIZE) {
  ...
  calculateProbs();
	...
	for (i=0; i<AL_SIZE; i++) {
		for (j=0; j<ali_len; j++) {
			profile_matrix[j][i] = log_probs ? log(probs[j][i]) : log(probs[j][i] / symbol_freqs[i]);
		} // end for
	} // end for

	if(arrgc==2) {
	  // '4' because at max '.prf' will be added
	  if ((profile_fname = (char *) calloc((strlen(argv[optind]) + 1 + 4), sizeof(char))) == NULL) {
	    printf("No memory left\n");
	    exit(1);
	  }
	  prepare_profile_fname(argv[optind], profile_fname);
	} // end if
	else {
	  if ((profile_fname = (char *) calloc((strlen(argv[optind+2]) + 1), sizeof(char))) == NULL) {
      printf("No memory left\n");
      exit(1);
    }
    strcpy(profile_fname, argv[optind+2]);
	} // end else
