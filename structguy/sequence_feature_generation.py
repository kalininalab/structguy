...
def initFeatures(config, samples):
    ...
    for db_name, feature_name_tag in dbs:
        samples.addFeature(f"Wildtype AA rate {feature_name_tag} {db_name}", "real", group="sequence", default_value=0.0)
        samples.addFeature(f"Mutant AA rate {feature_name_tag} {db_name}", "real", group="sequence", default_value=0.0, mutation_specific=True)
        samples.addFeature(f"Wildtype AA rate gapless {feature_name_tag} {db_name}", "real", group="sequence", default_value=0.0)
        samples.addFeature(f"Mutant AA rate gapless {feature_name_tag} {db_name}", "real", group="sequence", default_value=0.0, mutation_specific=True)
        samples.addFeature(f"MSA allel freq {feature_name_tag} {db_name}", "real", group="sequence", default_value=0.0)

        samples.addFeature(f"Other mutant AA rate {feature_name_tag} {db_name}", "real", group="sequence", default_value=0.0, mutation_specific=True)
        samples.addFeature(f"Other mutant AA rate gapless {feature_name_tag} {db_name}", "real", group="sequence", default_value=0.0, mutation_specific=True)
        samples.addFeature(f"PSIC wildtype AA {feature_name_tag} {db_name}", "real", group="sequence", default_value=0.0)
        samples.addFeature(f"PSIC mutant AA {feature_name_tag} {db_name}", "real", group="sequence", default_value=0.0, mutation_specific=True)
        samples.addFeature(f"dPSIC {feature_name_tag} {db_name}", "real", group="sequence", default_value=0.0, mutation_specific=True)

        samples.addFeature(f"Positional median dPSIC {feature_name_tag} {db_name}", "real", group="sequence", default_value=0.0)
        samples.addFeature(f"Window median dPSIC {feature_name_tag} {db_name}", "real", group="sequence", default_value=0.0)
        samples.addFeature(f"Protein median dPSIC {feature_name_tag} {db_name}", "real", group="sequence", default_value=0.0)
...
def getSequenceFeatures(config, samples, n_of_processes=6, update_mode=False):
        ...
        if returncode is None:
            for db in dbs:
                ...
                for line in lines:
                    ...
                    if gene not in sequence_maps[db]:
    ...
    msa_store = ray.put((msa_map, gpw_map))

    processes = {}
    for i in range(1, n_of_processes + 1):
        p = multiprocessing.Process(target=paraCalcSeqFeat, args=(config, lock, inqueue, outqueue, config.verbosity, msa_store))
        processes[i] = p
        p.start()
    for i in processes:
        processes[i].join()
...
def paraCalcSeqFeat(
    ...
    while True:
        ...
        for db_name, is_gpw in dbs:
            ...
            (psic_wt_map, psic_mut_map, dpsic_map, positional_dpsic_map, window_dpsic_map, protein_median_dpsic) = msa.calcPsicProfiles(config, u_ac, aacs, seed_seq, db_name, gpw=is_gpw, debug=debug)
