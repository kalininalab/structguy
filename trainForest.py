...
def trainClassificationForest(config,cv_slice,print_out=True,skip_scoring = False, remote = False, cv_counter = None):
    ...
    zero_scores = util.Scores(zero=True)
    ...
    scores_obj = util.Scores(acc = acc,roc = roc,precision = precision,recall = recall,f1 = f1,mcc = mcc)
...
@ray.remote(max_calls = 1)
def nested_ray_wrapper_for_conf_map_calculation(estimator, store):
    return featureAnalysis.calcConfusionMap(estimator, store)
...
def trainRegressionForest(config, cv_slice, samples = None, distance_map = None,print_out=True,skip_scoring = False, cv_counter = None, remote = False, overwrite_proc_n = None, debug = False, skip_feature_selection = False):
    ...
    if not skip_feature_selection:
        slice_updated = featureSelection.select_features(config, cv_slice, samples = samples, print_out = print_out, debug = debug, overwrite_proc_n = overwrite_proc_n)
    else:
        slice_updated = False
    ...
    if config.geometric_weighting:
        ...
        cv_slice.calcSubsampleDistanceWeights(config)
    ...
    scores_obj = util.Scores(mse = mse,r2 = r2,corr = corr,mcc = mcc,pearson_r=pearson, wmse = weighted_mse, wr2 = weighted_r2)
    ...
    if not cv_repeat:
        for i in range(0,repeat): #This can be used to ensure the robustness of the current parameter configuration
            ...
            if config.regression:
                forest, scores_obj, cv_counter, cv_slice = trainRegressionForest(config, cross_val_object, samples = samples, distance_map = distance_map, print_out = print_out,skip_scoring = skip_scoring, debug = debug, skip_feature_selection = skip_feature_selection)
            else:
                forest, scores_obj, cv_counter, cv_slice = trainClassificationForest(config,cross_val_object,print_out = print_out,skip_scoring = skip_scoring)
            if repeat > 1:
                if i == 0:
        ...
        slice_result_ids = []
        if remote:
            para_number = config.proc_n//len(cross_val_object.slices)
        for cv_counter in cross_val_object.slices:
            cv_slice = cross_val_object.slices[cv_counter]

            if remote:
                if config.regression:
                    slice_result_ids.append(trainRegressionForestWrapper.remote(config, cv_slice, samples = samples, distance_map = distance_map, print_out = print_out, cv_counter = cv_counter, debug = debug, skip_feature_selection = skip_feature_selection, overwrite_proc_n = para_number))
                else:
                    slice_result_ids.append(trainClassificationForestWrapper.remote(config, cv_slice,print_out = print_out, cv_counter = cv_counter))
            else:
                if config.regression:
                    slice_result_ids.append(trainRegressionForest(config, cv_slice, samples = samples, distance_map = distance_map, print_out = print_out, cv_counter = cv_counter, overwrite_proc_n = para_number, debug = debug, skip_feature_selection = skip_feature_selection))
                else:
                    slice_result_ids.append(trainClassificationForest(config,cv_slice,print_out = print_out, cv_counter = cv_counter))
